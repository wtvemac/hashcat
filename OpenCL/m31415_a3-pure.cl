...
KERNEL_FQ KERNEL_FA void m31415_mxx (KERN_ATTR_VECTOR ())
{
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos += VECT_SIZE)
  {
    ...
    _des_crypt_keysetup_wtv_vect (key[4],  key[0],  key_sched[0], key_sched[1], s_skb    );
    _des_crypt_decrypt_wtv_vect  (data[1], data[0], key_sched[0], key_sched[1], s_SPtrans);

    // DES pass 2, Enecrypt with second key
    _des_crypt_keysetup_wtv_vect (key[2],  key[3],  key_sched[2], key_sched[3], s_skb    );
    _des_crypt_encrypt_wtv_vect  (data[2], data[1], key_sched[2], key_sched[3], s_SPtrans);

    // DES pass 3, Decrypt with third key
    _des_crypt_keysetup_wtv_vect (key[0],  key[1],  key_sched[4], key_sched[5], s_skb    );
    _des_crypt_decrypt_wtv_vect  (data[3], data[2], key_sched[4], key_sched[5], s_SPtrans);

    DES_FP (data[3][1], data[3][0]);
    data[3][0] = hc_swap32(data[3][0]) ^ iv[0];
    data[3][1] = hc_swap32(data[3][1]) ^ iv[1];
